...
Классы `Annotate`, `Descriptor` должны быть доступны в глобальном неймспейсе при подключении заголовочного файла `reflect.hpp`. Этот заголовочный файл должен находиться в папке `task7`, расположенной в корне репозитория.

Код пушьте в ветку `task7` и делайте pull request в `master`.
